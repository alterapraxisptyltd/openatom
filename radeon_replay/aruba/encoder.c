			 ...
			 uint8_t bpc, uint32_t dp_link_rate)
{
	...
	if (!(aruba_read(rdev, CTL_VIDEO + off) & VIDEO_ON)) {
...
{
	uint32_t off;
	off = get_uniphy_reg_offset(0, encoder_id) << 2;
	//   0199: OR     reg[1cc9]  [..X.]  <-  01
	aruba_mask(rdev, (0x1cc9 << 2) + off, 0, 1 << 8);
	//   019e: DELAY_MicroSec  0a
	usleep(10);
	//   01a0: AND    reg[1cc4]  [...X]  <-  fe
	aruba_mask(rdev, CTL_VIDEO_2 + off, 1, 0);
...
void aruba_encoder_video_off(struct radeon_device *rdev, uint8_t encoder_id)
{
	...
	aruba_mask(rdev, CTL_VIDEO + off, 0xffff, 0);
	//   018d: DELAY_MicroSec  c8
	usleep(200);
	//   018f: OR     reg[1cc4]  [...X]  <-  01
	aruba_mask(rdev, CTL_VIDEO_2 + off, 0, BIT(0));
				   ...
				   uint8_t encoder_mode)
{
 ...
 l_less_common:
	...
	aruba_mask(rdev, (0x1bee + regptr) << 2, 0xff << 16, 0);
	//   00bc: CLEAR  reg[1bf9]  [XXXX]
	aruba_write(rdev, (0x1bf9 + regptr) << 2, 0);
	//   00c0: CLEAR  reg[1bed]  [XXXX]
	aruba_write(rdev, (0x1bed + regptr) << 2, 0);
